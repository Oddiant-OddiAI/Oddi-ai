...
class _Phase7TestReliability:
    """Deterministic reliability state used only by Phase 7 adapter tests."""

    def __init__(self):
        self.states = {}

    def is_healthy(self, provider, capacity_id=None):
        return self.states.get((provider, capacity_id), {}).get("healthy", True)

    def record_success(self, provider, capacity_id=None):
        self.states[(provider, capacity_id)] = {
            "healthy": True,
            "failure_count": 0,
        }
        return self.states[(provider, capacity_id)]

    def record_failure(self, provider, capacity_id=None, error=None):
        state = self.states.setdefault(
            (provider, capacity_id),
            {"healthy": True, "failure_count": 0},
        )
        state["failure_count"] += 1
        retryable = getattr(error, "retryable", True)
        return {
            "failure_type": "retryable" if retryable else "permanent",
            "healthy": True,
...
def test_phase7_retryable_failure_falls_back():
    from providers import adapters
    original_reliability = adapters.RELIABILITY_MANAGER
    adapters.RELIABILITY_MANAGER = _Phase7TestReliability()

    original = adapters.call_provider
    calls = []

    def fake_call_provider(provider, model, prompt, capacity_id=None, api_key_number=None):
    ...
    try:
        ...
        adapters.RELIABILITY_MANAGER = original_reliability
...
def test_phase7_timeout_classification():
    from reliability.manager import ReliabilityManager

    error = RuntimeError("request timed out")
    assert ReliabilityManager._classify_failure(error) == "timeout"


def test_phase7_rate_limit_classification():
    from reliability.manager import ReliabilityManager

    error = RuntimeError("HTTP 429: too many requests")
    assert ReliabilityManager._classify_failure(error) == "rate_limited"


def test_phase7_authentication_classification():
    from reliability.manager import ReliabilityManager

    error = RuntimeError("401 unauthorized")
    assert ReliabilityManager._classify_failure(error) == "authentication"


def test_phase7_explicit_retryable_classification():
    from reliability.manager import ReliabilityManager
    from providers.adapters import ProviderAdapterError
    ...
    assert ReliabilityManager._classify_failure(error) == "retryable"


def test_phase7_explicit_permanent_classification():
    from reliability.manager import ReliabilityManager
    from providers.adapters import ProviderAdapterError
    ...
    assert ReliabilityManager._classify_failure(error) == "permanent"


def test_phase7_failure_threshold_opens_circuit():
    from reliability.manager import ReliabilityManager
    ...
    assert manager.is_healthy("test_provider") is True
...
def test_phase7_success_resets_consecutive_failures():
    from reliability.manager import ReliabilityManager
    ...
    state = manager.record_success("test_provider")
...
def test_phase7_capacity_reliability_isolation():
    from reliability.manager import ReliabilityManager
...
def test_phase7_provider_reliability_isolation():
    from reliability.manager import ReliabilityManager
    ...
    assert manager.is_healthy("gemini") is False
    assert manager.is_healthy("mistral") is True


def test_phase7_cooldown_recovery():
    from reliability.manager import ReliabilityManager
...
def test_phase7_permanent_failure_opens_immediately():
    from reliability.manager import ReliabilityManager
    from providers.adapters import ProviderAdapterError
...
def test_phase7_failure_and_success_totals_are_preserved():
    from reliability.manager import ReliabilityManager
    ...
    manager.record_success("test_provider")
...
def main():
    ...
    tests = [
        ...
        # Phase 7 — Reliability Hardening
        ("P7: timeout classification", test_phase7_timeout_classification),
        ("P7: rate limit classification", test_phase7_rate_limit_classification),
        ("P7: authentication classification", test_phase7_authentication_classification),
        ("P7: explicit retryable classification", test_phase7_explicit_retryable_classification),
        ("P7: explicit permanent classification", test_phase7_explicit_permanent_classification),
        ("P7: failure threshold opens circuit", test_phase7_failure_threshold_opens_circuit),
        ("P7: success resets failures", test_phase7_success_resets_consecutive_failures),
        ("P7: capacity reliability isolation", test_phase7_capacity_reliability_isolation),
        ("P7: provider reliability isolation", test_phase7_provider_reliability_isolation),
        ("P7: cooldown recovery", test_phase7_cooldown_recovery),
        ("P7: permanent failure opens immediately", test_phase7_permanent_failure_opens_immediately),
        ("P7: failure/success totals", test_phase7_failure_and_success_totals_are_preserved),
    ]
